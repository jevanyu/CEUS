...
def map_to_gray_level(b,d):
    final_img = np.zeros(np.shape(img))
    print("hello")
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            if img[i,j] > 0 and b[i,j] > d[i,j]:
                final_img[i,j] = img[i,j]*((b[i,j]-d[i,j])/b[i,j])
            elif img[i,j] < 0 and b[i,j] < d[i,j]:
                final_img[i,j] = img[i,j]*((d[i,j]-b[i,j])/d[i,j])
            if final_img[i,j] < 0:
                final_img[i,j] = 0
            else:
                final_img[i,j] = final_img[i,j]
    return final_img
...
# map_to_gray_level
final_img = map_to_gray_level(b,d)
